...
public:
    ...
    matrix2D(int num_of_row, int num_of_col, long double ** base) {
        m = num_of_row;
        n = num_of_col;
        matrix = new long double *[m];
        for (int i = 0; i < m; i++) {
    ...
    matrix2D(vector<vector<long double> > base) {
        m = base.size();
        n = base.at(0).size();
        matrix = new long double *[m];
        for (int i = 0; i < m; i++) {
    ...
    matrix2D(int num_of_row, int num_of_col, matrix2D & base) {
        m = num_of_row;
        n = num_of_col;
        matrix = new long double *[m];
        for (int i = 0; i < m; i++) {
    ...
    void set_value(vector<long double> & base) {
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                matrix[i][j] = base[i];
            }
    ...
    void set_value(vector<long double> & base1,vector<long double> base2) {
        for (int i = 0; i < base1.size(); i++) {
                matrix[i][0] = base1[i];
        }
        for (int i = 0; i < base2.size(); i++) {
                matrix[i+base1.size()][0] = base2[i];
        }
    }
    void set_value(long double ** base) {
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                matrix[i][j] = base[i][j];
            }
    ...
    void set_value(int i,int j,long double value){
        if(i>=m || j>=n)
            throw runtime_error("matrix has not this i j");
        matrix[i][j] = value;
    }
    void set_value_random() {
        random_device rd;
        mt19937 mt(rd());
        uniform_real_distribution<>rnd(-0.5,0.5);
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                matrix[i][j] = rnd(mt);
            }
    ...
    void set_value_random(long double a,long double b) {
        random_device rd;
        mt19937 mt(rd());
    ...
    long double get_value(int i, int j) {
        if(i>=m || j>=n)
            throw runtime_error("i or j not in matrix");
        return matrix[i][j];
    }
    long double get_sum() {
    ...
    vector<long double> get_vector_1D(){
        vector<long double> res(m);
        for(int i=0;i<m;i++)
            res[i] = (matrix[i][0]);
        return res;
    }

    vector<long double> get_vector_1D(vector<long double> & res){
        for(int i=0;i<m;i++)
            res[i] = (matrix[i][0]);
    }
    ...
    friend ostream &operator<<( ostream &output,const matrix2D &mat){
        output << "row:" << mat.m << " col:" << mat.n << endl;
        output << "---------------------" << endl;
    ...
    void operator *=(long double ** array){
        zarbdot(matrix, array, matrix, m, n);
    }
    void operator *=(matrix2D & array){
    ...
    void multiply_dot_matrix_to_matrix_onthis(matrix2D & matrix1,
                                              matrix2D & matrix2) {
        zarbdot(matrix1.get_array(), matrix2.get_array(), matrix,
    ...
    void multiply_matrix_to_matrix_onthis(matrix2D & matrix1,
                                          matrix2D & matrix2) {
        zarb(matrix1.get_array(), matrix2.get_array(), matrix,
             matrix1.get_num_of_row(), matrix1.get_num_of_col(),
